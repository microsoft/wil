...
TEST_CASE("BasicRegistryTests::enumerate_keys", "[registry]]")
{
    ...
    SECTION("enumerate_keys with many subkeys - std::for_each and std::count usage")
    {
        ...
        std_count = std::count(
            wil::reg::enumerate_keys(wil::reg::open_shared_key(HKEY_CURRENT_USER, testSubkey)).begin(),
            wil::reg::enumerate_keys(wil::reg::open_shared_key(HKEY_CURRENT_USER, testSubkey)).end(),
            test_enum_KeyName1);
        REQUIRE(std_count == 1);
