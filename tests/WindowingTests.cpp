...
TEST_CASE("EnumWindows", "[windowing]")
{
    ...
    REQUIRE_THROWS_AS(wil::for_each_window([](HWND)
        {
            throw std::exception();
        }), std::exception);
}
...
TEST_CASE("EnumThreadWindows", "[windowing]")
{
    ...
    // throwing version
    {
        ...
        // with exceptions
        REQUIRE_THROWS_AS(wil::for_each_thread_window(thread_id, [](HWND)
            {
                throw std::exception();
            }), std::exception);
    }
...
TEST_CASE("EnumChildWindows", "[windowing]")
{
    ...
    // throwing version
    {
        ...
        // with exceptions
        REQUIRE_THROWS_AS(wil::for_each_child_window(parent, [](HWND)
            {
                throw std::exception();
            }), std::exception);
    }
