...
public:
    ...
    constexpr path_view_t(const_pointer data, size_type length) noexcept : m_data(data), m_length(length)
    {
    }

    constexpr path_view_t(const_pointer data) noexcept : m_data(data), m_length(data ? ::wcslen(data) : 0)
    {
    }
    ...
    constexpr path_view_t(StringViewLike&& str) noexcept : path_view_t(str.data(), str.length())
    {
    }

    // std::string overload (disallows r-values, i.e. temporaries)
    template <typename StringLike, wistd::enable_if_t<details::is_string_like<wistd::remove_reference_t<StringLike>, wchar_t>, int> = 0>
    constexpr path_view_t(StringLike& str) noexcept : path_view_t(str.data(), str.length())
    {
    }
    ...
    {
        return details::npos / sizeof(value_type);
    }
    ...
    constexpr void swap(path_view_t& other) noexcept
    {
    ...
    constexpr path_view_t substr(size_type pos = 0, size_type count = npos) const noexcept
    {
        ...
        auto len = (wistd::min)(count, m_length - pos);
        return path_view_t(m_data + pos, len);
    }
    ...
    /*constexpr*/ int compare(const wchar_t* str) const noexcept
    {
        ...
        return compare(path_view_t(str));
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return starts_with(path_view_t(str));
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return ends_with(path_view_t(str));
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return find(path_view_t(str), pos);
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return rfind(path_view_t(str), pos);
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return find_first_of(path_view_t(str), pos);
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return find_last_of(path_view_t(str), pos);
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return find_first_not_of(path_view_t(str), pos);
    }
    ...
    /*constexpr*/ size_type find_first_not_of(wchar_t ch, size_type pos = 0) const noexcept
    {
        ...
        return find_first_not_of(path_view_t(&ch, 1), pos);
    }
    ...
    {
        // str may not actually be a path, but we don't do any validation and this properly handles null
        return find_last_not_of(path_view_t(str), pos);
    }

    /*constexpr*/ size_type find_last_not_of(wchar_t ch, size_type pos = npos) const noexcept
    {
        // ch is pretty clearly not a path, but we don't do any validation
        return find_last_not_of(path_view_t(&ch, 1), pos);
    }
